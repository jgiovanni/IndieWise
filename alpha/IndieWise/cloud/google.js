...
var googleClientId = '322274582930-4m1dueb708gvdic28n12e5dhqq121a6b.apps.googleusercontent.com'; //The client ID obtained from the Google Developers Console, follow the steps above
var googleClientSecret = '6kjGiH7lv9fCoXzf3HjS4X3f'; //The client secret obtained from the Google Developers Console, follow the steps above

var googleValidateEndpoint = 'https://www.googleapis.com/oauth2/v3/tokeninfo'; //this is the only verification link you need to verify the user's Google Access Token
...
Parse.Cloud.define('accessGoogleUser', function(req, res) {
    ...
    Parse.Promise.as().then(function() {
        ...
        var userData = JSON.parse(httpResponse.text);

        if (userData && userData.aud === googleClientId) {
            return upsertGoogleUser(data.code, userData, data.email);
        } else {
...
var getGoogleAccessToken = function(code) {
    ...
    return Parse.Cloud.httpRequest({
        url: googleValidateEndpoint + '?id_token=' + code
    });
}
...
var upsertGoogleUser = function(accessToken, googleData, emailId) {
    ...
    console.log("sub: " + googleData.sub);
    query.equalTo('accountId', googleData.sub);
...
var newGoogleUser = function(accessToken, googleData, email) {
    ...
    return user.signUp().then(function(user) {
        ...
        ts.set('user', user);
        ts.set('accountId', googleData.sub);
...
Parse.Cloud.afterSave("Message", function(request) {
    var currChannel = request.object.get("currChannel");
    var query = new Parse.Query(Parse.Installation);
    query.equalTo('channels', currChannel); // Set our channel
