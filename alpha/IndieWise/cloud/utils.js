...
function serializeId(parseObject) {
     ...
     */
    return _.isString(parseObject) ? 'ref:' + parseObject : 'ref:' + parseObject.className + ':' + parseObject.id;
};
...
exports.parseToActivity = function parseToActivity(parseObject) {
    ...
    activity.object_data = parseObject.get('object_data');
    ...
    _.each(activities, function(activity) {
        ...
        _.each(activity, function(value, field) {
            if (field !== 'object_data') {
                ...
                if (_.isUndefined(p1)) {
                    if (field !== 'object_data' && value && !_.isEmpty(value) && value.indexOf('ref') === 0) {
...
exports.countWatched = function countWatched(parseObject) {
    /*var verb = parseObject.get('verb');
    if (verb === 'watch') {
        var query = new Parse.Query("Watched");
        var obj = parseObject.get('object');
        query.equalTo("film", {__type: "Pointer", className: "Film", objectId: obj.id});
        query.get().then(function (res) {
            res.increment("count");
            res.save();
        }, function (res){
            console.log('countWatched: here');
            var Watched = Parse.Object.extend("Watched");
            var watched = new Watched();
            watched.set('film', obj);
